...
public class EmetteurMultiTrajets extends Transmetteur<Float,Float>{
	...
	public void recevoir(Information<Float> information) throws InformationNonConformeException {
		...
		// on rajoute l'information utile puis un nombre de 0 égale à la valeur du plus grand décalage
		...
		// on veut maintenant respecter r(t) = s(t) +Aks(t-Tk) + b(t) , k appartenant à [|0;5|]
		
		// alphaCourant va nous servir à récupérer le alpha correspondant au décalage observé
		int alphaCourant = 0;		
		...
		for (int tau : listTaus) { // on va récupérer le signal initial, et le rajouter par décalage
			...
			for(int pos=0 ; pos<informationTi.nbElements() ; pos++) {
				if(pos>=tau && pos<informationTi.nbElements()) { // quand l'on atteint le premier décalage
					...
					// et on remplace à l'endroit où l'on est par l'échantillon initial + l'échantillon du signal
		...
		this.emettre(); // on emet ensuite le nouveau signal
	}
